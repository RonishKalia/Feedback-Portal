...
//	error_reporting(error_reporting() & ~E_DEPRECATED);
	...
	function registerFormValidate ($email, $username, $passwd, $cpasswd) 
	{
	      $msg = "";
...
// 	      }
	...
	function addAccount($username, $password, $email) //Create a new account
	{
		...
		$sql = "INSERT INTO user_details(user_id, added_on) VALUES(:user_id, NOW())";
	...
	function saveProfile($user_id, $name, $college_id, $contact, $profession, $gender, $accommodation, $address) //Update any changes to profile
	{
		$x = 10000 + $user_id;
		$reg_id = "EFFE2013" . $x;
		$lock = 1;
		$sql = "UPDATE user_details SET name = :name, college_id = :college_id, contact = :contact, profession = :profession, gender = :gender, modified_on = NOW(), accommodation = :accommodation, address = :address, reg_id = :reg_id, locked = :lock WHERE user_id = :user_id AND isdeleted = 0";
		$params = array(':user_id' => $user_id, ':name' => $name, 'college_id' => $college_id, 'contact' => $contact, ':profession' => $profession, ':gender' => $gender,':accommodation' => $accommodation, ':address' => $address, ':reg_id' => $reg_id, ':lock' => $lock);
		$result = DatabaseHandler::Execute($sql, $params);
	}
